...
public class MkmWantListPanel extends JPanel {
	...
	private void initGUI() {
		...
		btnDeleteWl.addActionListener(ae -> {
			if (selected == null) {
				JOptionPane.showMessageDialog(null, "Need to select a WantList", MkmConstants.MKM_ERROR,
						JOptionPane.ERROR_MESSAGE);
				return;
			} else {
				int res = JOptionPane.showConfirmDialog(null, "delete " + selected + " ?", "Confirmation",
				...
				if (res == JOptionPane.YES_OPTION) {
					try {
						...
						JOptionPane.showMessageDialog(null, e1.getMessage(), MkmConstants.MKM_ERROR,
								JOptionPane.ERROR_MESSAGE);
					}
				}
			}
	...
	protected void loadWantList() {
		...
		try {
			...
			log.error("error loading wanlist", e);
