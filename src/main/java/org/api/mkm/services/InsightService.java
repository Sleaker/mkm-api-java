...
public class InsightService {
	...
	private double parsePrice(String strprice) {
		try {
			...
			log.error("error parsing " + strprice + " : " + e);
			return 0.0;
		}
	}
	...
	public Map<Date, Double> priceHistory(Product p, boolean foil) throws IOException {
		...
		JsonArray values = el.getAsJsonObject().get("data").getAsJsonObject().get("datasets").getAsJsonArray().get(0)
				.getAsJsonObject().get("data").getAsJsonArray();

		for (int i = 0; i < dates.size(); i++) {
			try {
				ret.put(new SimpleDateFormat("dd.MM.yyyy").parse(dates.get(i).getAsString()),
						values.get(i).getAsDouble());
			} catch (IndexOutOfBoundsException ioobe) {
				// do nothing
			} catch (ParseException e) {
				log.error("Error parsing " + dates.get(i).getAsString(), e);
			}
		}
		return ret;
	}
